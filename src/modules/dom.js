...
const dom = (() => {
	...
	function renderProjects() {
		...
		for (let i = 0; i < projects.projectsList.length; i++) {
			document.querySelector("#projectsDom").innerHTML += /*html*/ `
                <button id="projectsText" class="btn btn-outline font-bold">${projects.projectsList[i].title}</button>
            `;
		}
	}
	...
	function renderTodos() {
		...
		for (let i = 0; i < projects.projectsList[0].tasks.length; i++) {
			document.querySelector("#todolist").innerHTML += /*html*/ `
                    <div id="todo" class="flex flex-row justify-between w-full bg-primary rounded-md p-2">
                        ...
                        <div id="todoContent" class="flex flex-col flex-1 mx-2 text-primary-content">
                            <span id="todoText" class="font-bold">${projects.projectsList[0].tasks[i].title}</span>
                            <span id="todoDesc" class="text-xs">${projects.projectsList[0].tasks[i].description}</span>
                        </div>
                        <div id="actionButtons" class="flex flex-row gap-2">
                            ...
                            <button class="btn" id="todoDelete">
